...
import { deleteCharacter } from "~/server/action";
...
export default function DeleteCharacterModal({
  ...
  const handleDelete = async () => {
    try {
      ...
      router.refresh();
      toast.success("successfully delete character!");
    } catch (error: unknown) {
      const Error: ErrorInterface = {
        message: (error as Error).message || "Internal Server Error",
      };
      toast.error(Error.message);
      console.error("Error while delete character:", error);
    } finally {
      setloading(false);
      setOpen(false);
    }
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  ...
                  >
                    {loading ? <VoiceLoadSpiner /> : "Delete"}
                  </button>
                  <button
                    type="button"
